...
- New (admin): release a new version to some people first. "Who gets it" on the publish card: Everyone, or Only some people - search, click a department to tick all its people (it shows when only some are ticked), tick people one by one, "Select shown" / "Clear". Everyone else doesn't see it at all. People who already have that version don't get notified again when you release it to everyone. When it's fine, press "Release to everyone" (with or without the pop-up).
- New: the Task files tab is now "Clients" (files and notes), with a clearer layout: each client is a card, and an open one is highlighted with separate Files and Notes sections. Files: Open (images, PDF, web pages and text open in a tab; other files download), Show in folder (the files are stored inside Chrome, so the first time it saves a copy to Downloads > Personal ClickUp Manager > Clients > <client> and opens that folder; after that it just opens the same copy) and delete. Files added from now on keep the original (up to 25 MB) so they open as they are; older ones open as the text that was read from them. "☁ Copy to Drive" puts a client's files in My Drive > Personal ClickUp Manager > Clients > <client> (the automatic backup stays in Drive's hidden app data, text only). Notes: write what you were told or what to watch out for, attach screenshots (Ctrl+V) or files, tick "⏰ Remind me at" to get a reminder at a date and time (a note's screenshot shows in the reminder too), and edit or delete them. A task's details show its client's notes and "Explain this task" uses them. Notes are backed up to Google Drive with your settings (their attachments stay on this computer).
- Improved: the side panel button has a proper icon (instead of an arrow) that looks right in light and dark mode.
- New: in Help & issues threads you can react to any message (👍 ❤️ 😂 🎉 😮 🙏 ✅ 👀; point at a reaction to see who) and reply to a specific message (Reply shows a quote of what you're answering). Loading no longer blanks the tab: what's on screen stays while it updates ("Updating…"), your reply shows up straight away, and a reply you're typing is kept when the thread refreshes. (Admins: this needs the updated hub script - redeploy it once.)
- New: Help & issues. A new tab where everyone can report a problem or ask a question and see what's already been reported: search first, press "Me too" instead of posting a duplicate, paste screenshots (Ctrl+V) or drop files, and include diagnostics (only the admin sees them; no passwords, tokens or client names). The admin replies in the same thread, can mark it "Resolved - fixed in v…", and everyone in the thread (including "Me too") gets a notification; after updating, a fixed issue shows "✓ Fixed in your version". Names and photos come from each person's ClickUp account. The admin can pin, lock, rename or delete threads, edit or delete any message, mute or ban someone, turn on slow mode and choose whether screenshots are visible to everyone. It all runs on a small Google Apps Script in the admin's own Google account (set up once in Admin > Team hub; everyone else's copy gets it automatically).
- New (admin): Admin > Team hub shows how many people use the extension, who was active today and this week, each person's version (older ones highlighted) and when they last used it. Each copy checks in about once a day with its ClickUp name and photo, version and last-active time; the Help & issues tab says so.
- Fixed: Agent Router logins that need you (passkey, Google sign-in, CAPTCHA) kept showing the red "finish in the open tab" message even after you finished the login yourself. The extension now watches the tab it left open and, once you're logged in there, records the login like a normal run (today's credit included) and clears the message. It never credits the account if that tab is logged in to a different Agent Router account. The message also has two buttons now: "✓ I logged in" (mark today's login as done) and "Dismiss" (just hide the message).
- Fixed: in the floating tracker, with no timer running, buttons (Start Extra Task, the ▶ next tasks, Resume last) sometimes needed a second click. The tracker redrew itself every second, and a redraw between pressing and releasing the mouse swallowed the click. It now only redraws when something actually changes.
- Improved: actions that wait for ClickUp say so. Starting, stopping or completing a task from the popup, side panel or dashboard shows a spinner in the button and "Starting… / Stopping… / Completing… waiting for ClickUp" under the task, changing to "ClickUp is slow to answer - still working…" after 6 seconds, so a slow reply never looks like a broken button. The Stop / Complete buttons in Tracking now, the Extra Task button and the floating tracker's buttons say "Stopping…", "Completing…" or "Starting…" instead of "…".
- New: Reminders. Set your own reminders, about a task or anything else, with the ⏰ button next to the bell (popup, side panel and dashboard), "⏰ Remind me" in a task's details (the task is linked), or the new Reminders tab in Options. Pick a time (In 15 min, In 1 hour, Tomorrow 9:00, Monday 9:00, or any date and time) and optionally repeat it every day, on weekdays or every week. The reminder pops up with Snooze 10 min and Done; clicking it opens the linked task. Reminders show even while notifications are paused (only "All notifications" off silences them) and follow your Sound and Volume settings. They're backed up to Google Drive with your settings. Chrome has to be open for a reminder to pop up; one that came due while it was closed shows as "Missed reminder" when Chrome starts. The ⏰ shows how many are due today.
- Fixed: turning notifications off (or "Pause 1 hour") in the bell menu didn't silence everything. Update notices, the "set up automatic updates" notice and the Google Drive full warning still popped up, and the update notice still played its sound. Now nothing pops up or plays while notifications are off or paused. The only exception is a critical update, which still appears, but silently. A held-back setup notice shows once notifications are back on.
- New: notification volume. A Volume slider (5-100%) in Options > General > Notifications and sound, and under Sound in the bell menu, sets how loud every notification sound is: the chime, the danger alarm, the milestone sound and your own sounds. Letting go of the slider plays a sample at that level, and Test sound uses it too. 100% is your computer's normal volume; to go louder than that, raise the computer's volume.
- New: Agent Router "Keep me signed in to GitHub" (Options > Agent Router, off by default). For people with more than one GitHub account saved in GitHub's account switcher: the daily login no longer signs you out of GitHub. Instead it asks GitHub to show its account picker and chooses the right account for each Agent Router account. An account that isn't signed in yet is added with its saved password and 2FA code, without signing the others out. As a safety check, if GitHub is signed in as a different account than expected, it stops and asks you instead of logging into Agent Router with the wrong account.
- New: Agent Router Debug > Test login has an "All accounts (one after another)" choice, to watch every account log in in turn (for example to see "Keep me signed in to GitHub" switch GitHub accounts without signing out). Like the single test it doesn't change the daily checkpoint, so any account whose 24 hours are already up is skipped with a note: logging it in would earn that day's $25 without recording it, and the normal Run handles that one properly. Like Run, each account's tab closes as soon as it has logged in (a tab that needs you stays open).
- New: attach screenshots and files to a reminder. Paste a screenshot with Ctrl+V, drop files on the reminder form or use 📎 Attach (up to 10 files, 20 MB each). When the reminder pops up, a screenshot shows right in it, and clicking it opens the reminder in Options > Reminders with its files one click away (images and PDFs open in a tab, other files download). Attached files stay on this computer (the reminders themselves are backed up to Drive).
